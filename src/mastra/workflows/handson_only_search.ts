...
export const handsonWorkflow = createWorkflow({
    id: "handsonWorkflow",
    description: "自然言語の質問からConfluenceのページを検索して、内容を要約して回答します",
    ...
    outputSchema: z.object({
        answer: z.string().describe("要約された回答"),
    }),
})
    .then(
        createStep({
            ...
            execute: async ({ inputData, getInitData }) => {
                ...
                if (error || !page || !page.content) {
                    return {
                        ...
                        pageTite: page?.title || "不明",
                        pageUrl: page?.url || "",
                    };
                }
                // エージェントへの指示を作成
                const prompt = `
以下のConfluenceページの内容に基づいて、ユーザーの質問に答えて下さい。
...
回答は完結で分かりやすく、必要に応じて箇条書きを使用してください。`;
